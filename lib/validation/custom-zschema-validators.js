...
function customValidatorFn (report, schema, json) {
  ...
  if (this.validateOptions && !shouldSkipValidate(this.validateOptions, ['SECRET_PROPERTY'])) {
    // x-ms-secret exists in the property
    if (schema['x-ms-secret'] && schema['x-ms-secret'] === true && json !== undefined) {
      // currently, we only care about the string value
      if (typeof schema.type === 'string' && isResponse) {
        let propertyName = ''
        let errorMessage = 'Property {0} has ms-secret value'
