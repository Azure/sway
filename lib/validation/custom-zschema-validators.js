...
function requiredPropertyValidator (report, schema, json) {
  ...
  while (idx--) {
    ...
    isWriteOnlyProperty = (schema.properties && schema.properties[`${requiredPropertyName}`]) && schema.properties[`${requiredPropertyName}`].writeOnly;
    ...
    if (
      ...
      ((xMsMutability &&
        xMsMutability.indexOf('read') === -1) || isWriteOnlyProperty)
    ) {
...
function writeOnlyValidator (report, schema, json) {
  // http://json-schema.org/latest/json-schema-validation.html#rfc.section.9.4
  ...
  if (isResponse && schema && schema.writeOnly && json !== undefined) {
    let errorMessage = 'WriteOnly property `"{0}": '
